...
export const Icon = ({ name, size = 24, color = 'currentColor', strokeWidth = 2, className = '' }) => {
  const p = {
    ...
    settings:'M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6zM19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z',
...
export function FilledButton({ children, onClick, disabled, style = {}, className = '' }) {
  return (
    <button onClick={onClick} disabled={disabled} className={`gr-press ${className}`} style={{
      height: 40, padding: '0 24px', borderRadius: 20, border: 'none',
      background: disabled ? 'var(--md-sys-color-surface-container-high)' : 'var(--md-sys-color-primary)',
      color: disabled ? 'var(--md-sys-color-on-surface-variant)' : 'var(--md-sys-color-on-primary)',
      ...TYPE.labelL, cursor: disabled ? 'default' : 'pointer', ...style,
    }}>{children}</button>
  );
}

export function TonalButton({ children, onClick, style = {}, icon, className = '' }) {
  return (
    <button onClick={onClick} className={`gr-press ${className}`} style={{
...
export function TextButton({ children, onClick, style = {}, className = '' }) {
  return (
    <button onClick={onClick} className={`gr-press ${className}`} style={{
...
export function OutlinedButton({ children, onClick, style = {}, className = '' }) {
  return (
    <button onClick={onClick} className={`gr-press ${className}`} style={{
...
export function IconButton({ icon, onClick, background, size = 40, style = {}, className = '' }) {
  return (
    <button onClick={onClick} className={`gr-press ${className}`} style={{
...
export function FAB({ icon, label, onClick, extended, style = {}, className = '' }) {
  return (
    <button onClick={onClick} className={`gr-press ${className}`} style={{
...
export function Chip({ children, selected, onClick, leading, style = {}, className = '' }) {
  return (
    <button onClick={onClick} className={`gr-press ${className}`} style={{
...
export function Card({ children, elevated, style = {}, onClick, className = '' }) {
  return (
    <div onClick={onClick} className={className} style={{
      borderRadius: 12,
      background: elevated ? 'var(--md-sys-color-surface-container-low)' : 'var(--md-sys-color-surface-container-highest)',
...
export function Avatar({ rider, size = 40, ring, style = {}, className = '' }) {
  return (
    <div className={className} style={{
      ...
      flexShrink: 0,
      boxShadow: ring ? `0 0 0 3px var(--md-sys-color-surface), 0 0 0 5px ${ring}` : 'none',
      position: 'relative',
      ...style
    }}>{rider.initial}</div>
  );
...
export function BottomSheet({ children, style = {}, className = '' }) {
  return (
    <div className={className} style={{
      ...
      <div style={{
        ...
        background: 'var(--md-sys-color-on-surface-variant)', opacity: 0.4,
        margin: '0 auto 12px',
      }}/>
      {children}
    </div>
...
export function Segmented({ options, value, onChange }) {
  return (
    <div style={{ display: 'flex', border: `1px solid var(--md-sys-color-outline)`, borderRadius: 20, overflow: 'hidden' }}>
      {options.map((o, i) => {
        const selected = o.value === value;
        return (
          <button key={o.value} onClick={() => onChange(o.value)} className="gr-press" style={{
            flex: 1, height: 40, border: 'none',
            borderLeft: i === 0 ? 'none' : `1px solid var(--md-sys-color-outline)`,
            background: selected ? 'var(--md-sys-color-secondary-container)' : 'transparent',
            color: selected ? 'var(--md-sys-color-on-secondary-container)' : 'var(--md-sys-color-on-surface-variant)',
            ...TYPE.labelL, cursor: 'pointer',
...
export function BatteryGlyph({ level }) {
  const lvl = Math.max(0, Math.min(1, level / 100));
  const warn = level < 25;
  return (
    <svg width="18" height="12" viewBox="0 0 18 12">
      <rect x="0.5" y="0.5" width="14" height="11" rx="2" fill="none" stroke="var(--md-sys-color-on-surface-variant)" strokeWidth="1"/>
      <rect x="15" y="4" width="2.5" height="4" rx="1" fill="var(--md-sys-color-on-surface-variant)"/>
      <rect x="2" y="2" width={11 * lvl} height="8" rx="1"
            fill={warn ? 'var(--md-sys-color-error)' : 'var(--md-sys-color-primary)'}/>
    </svg>
  );
}
